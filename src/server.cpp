		...
		switch (c->state)
		{
			case conn_listening:
				...
				if ((sfd = accept(c->sfd, (struct sockaddr *) &addr, &addrlen)) == -1)
				{
					...
					{
						if (settings.verbose > 0) fprintf(stderr,
...
static int server_socket(const int port, const bool is_udp)
{
	...
	if (error != 0)
	{
		if (error != EAI_SYSTEM) fprintf(stderr, "getaddrinfo(): %s\n",
				gai_strerror(error));
		else perror("getaddrinfo()");
		...
		const int read_buffer_size, const bool is_udp, struct event_base *base)
{
	...
	{
		if (init_state == conn_listening) fprintf(stderr, "<%d server listening\n",
				sfd);
		else if (is_udp) fprintf(stderr, "<%d server listening (udp)\n", sfd);
		else fprintf(stderr, "<%d new client connection\n", sfd);
	}
...
static void process_server(conn *c, char *command)
{
	...
	if (nLen != 0) out_stringn(c, buf, nLen);
